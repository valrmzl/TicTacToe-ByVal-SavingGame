...
{
    //printf("El juego se guardara\n");
    FILE *guardar= fopen("saveGato.txt","r+b");
    if(guardar!=NULL)
    {
        printf("Hay un juego prviamente cargado\n");
        printf("Cargando...\n");
        g= malloc(sizeof(Game));
        fread(g,sizeof(Game),1,guardar);
        fclose(guardar);
    }
    else
    {
        printf("No hay juegos cargados previamente\n");
        //printf("Se ha guardado el juego por primera vez\n");
        g=newGame();
    }
    return g;
...
void saveGame(Game *g,int place)
{
    ...
    {
        printf("Guardando...\n");
        printf("El juego se ha guardado con exito!\n");
        if(g->currentPlayer=='o')
            g->currentPlayer='x';
        else
            g->currentPlayer='o';

        FILE *guardar= fopen("saveGato.txt","w+");
        if(guardar)
        {
